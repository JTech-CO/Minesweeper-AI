...
# ── 웹 통합 관리 대시보드(권장): 브라우저가 자동으로 열림 ──
# 가장 쉬운 방법: 파일 탐색기에서 server\dashboard.bat 을 더블클릭
# 또는 터미널에서 (반드시 .venv\Scripts\python.exe 접두사 포함):
.venv\Scripts\python.exe -m trainer.dashboard   # http://127.0.0.1:8800
# 학습 일시정지/재시작 · 라이브 하이퍼파라미터(lr/batch/train-freq/ε) · GPU 모니터(VRAM/온도/클럭)
# · 병렬 플레이 1~8보드 + 난이도 선택 · 누적 학습 세션수(영속) · 승률 곡선
# 학습은 대시보드 인프로세스로 관리되며, beginner 체크포인트에서 이어받아 재개됩니다.
