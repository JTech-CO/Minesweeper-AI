...
## 현재 상태
- **현재 Phase**: **M4 RL 능동 튜닝 재개**(사용자 목표 50%). 학습이 0.20 정체에서 **2.85M ep·eval ~0.35**(best)로 상승 — beginner 천장 ~0.85이므로 0.50은 도달 가능 범위(Expert 40% 상한과 별개). 단 ~0.30-0.35에서 정체 기미(레시피=reveal-maximizer 보상 추정).
- **튜닝 결정(2026-06-05): 파인튜닝 먼저, 한계면 구조 업그레이드**. 1단계 적용: **lr 후반 감쇠**(effective=cfg.lr×max(0.2,1−step/32M) → 22.4M에서 ~1.5e-4) + nan status 버그 수정 + 대시보드 lr 표시. 검증(격리 복사본 resume). 2단계(미적용): 보상 재가중/구조 업그레이드(width128·blocks6·Dueling)는 escalation.
- **플레이는 솔버-하이브리드/NG가 정본**(아래). 모델 단독 0.35는 시연엔 불필요하나 사용자가 모델 품질 향상을 원함.
- **플레이 승률(솔버 하이브리드, 측정)**: 표준 랜덤 보드 beginner **0.948** / intermediate **0.820** / expert **0.370**(표준 강제-추측 상한). 모델 단독은 beginner 0.193.
- **NG(무추측) 모드**: 대시보드 보드에 NG 토글 — 추측 없이 풀리는 보드만 생성 + 배치 솔버 → **전 난이도 100% 클리어**(검증 100/60/40). 일관 시연용.
- **온라인 실사이트(minesweeper.online) 기능 전부 제거(2026-06-05)**: 비로그인 무한 플레이가 DDoS로 인식돼 **IP밴**. `trainer/online.py` 삭제, 대시보드의 `/api/online`·CDP·Chrome 런처·UI 패널·playwright 의존성 제거. **로컬 전용**(자체 생성 보드만). 가드레일상으로도 외부 사이트 접속 코드가 사라짐.
- **마지막 갱신**: 2026-06-05
...
- **2026-06-05**: **온라인 실사이트 기능 전부 제거**(IP밴 — DDoS 오인) → 로컬 전용. `online.py` 삭제 + 대시보드 online/CDP/Chrome 코드·UI·playwright 의존성 제거. **win-rate 패널을 3그래프로 분리**(eval%=파랑·train%=노랑·lifetime sessions=초록, x=eval샘플 인덱스=시간) + live hyperparameters 컬럼 폭 확대(1.1fr). 서브 스모크(부팅·3캔버스·online 무참조·status 200)·pytest 25·ruff 그린.
